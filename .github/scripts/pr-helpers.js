  ...
  triage: {
    ...
    DNS_VERIFY_INSTRUCTIONS(isDelete, hostname, expected) {
      return [
        ...
        `方法二（文件）：在网站根目录放置 xia-verification.html，内容：${expected}`,
        '完成后 push 更新以触发重新校验。',
      ].join('\n');
    },
  },
...
function buildFooter(runUrl) {
  const retryHint =
    '💡 如果因网络波动等原因多次不通过，可尝试 **关闭后重新打开 PR** 以再次触发检查。仍然不行请联系 [ERX399](https://github.com/ERX399)（在[仓库首页](https://github.com/ERX399/friends-data)点击 About 可以找到联系方式）。';
